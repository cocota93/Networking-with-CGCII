...
/**
 ...
 @class		CListDispatchable
 @brief		배열형으로 멤버를 관리하는 그룹 클래스로 자리를 자동 결정해준다.
 @todo		
 @ref		
 @details	\n
 CList에 Dispatchable 기능을 추가한 객체이다.
 Member가 Dispatchable을 상속받을 경우 자동으로 Register와 Unregister를 수행해준다.
...
// Inherited Classes) 
// ----------------------------------------------------------------------------
	virtual public				CList<_TMEMBER, _TMEMBER_DATA, _TMEMBERBASE>,	//     Group Base
...
// ----------------------------------------------------------------------------
public:
			CListDispatchable(int _nMaxMember=INT_MAX) : BASEGROUP(_nMaxMember)	{}
	virtual	~CListDispatchable()									{}
...
void CListDispatchable<_TMEMBER, _TMEMBER_DATA, _TMEMBERBASE>::_ProcessMemberEntered(TMEMBER* _pMember, CGMSG& _Msg)
{
...
template <class _TMEMBER, class _TMEMBER_DATA, class _TMEMBERBASE>
uintptr_t CListDispatchable<_TMEMBER, _TMEMBER_DATA, _TMEMBERBASE>::_ProcessMemberLeaving(TMEMBER* _pMember)
{
	// Declare) 
