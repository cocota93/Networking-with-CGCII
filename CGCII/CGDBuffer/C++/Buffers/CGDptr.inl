...
inline void* ptr::_prepend_bytes(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ size_t _Size, _In_reads_bytes_(_Size) const void* _Buffer)
{
...
void* ptr::_prepend_string_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest)
{
#ifdef _WIN32
	// Check) _String이 nullptr이면 안된다.
	...
	// 6) [문자열_길이]를 써넣는다.
	*reinterpret_cast<COUNT_T*>(p)	 = lengthString;
...
template <class T, class... TREST>
size_t ptr::_prepend_text_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest)
{
#ifdef _WIN32
	// Check) _String이 nullptr이면 안된다.
	CGD_ASSERT(_Format!=nullptr, throw std::invalid_argument("_Format is nullptr! [0] (" __FUNCTION__ ")"));
	...
	// 2) [원본_버퍼_시작_크기]에 [형식_문자열]를 써넣는다.
	size_t	lengthString	 = _Xsprintf(pStringStart, _Format, std::forward<TREST>(_Rest)...);
	size_t	sizeString		 = lengthString*sizeof(T);
...
inline void* ptr::_prepend_buffer(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const buffer& _Buffer)
{
	...
	// Check) _Data이 nullptr이면 안된다.
	CGD_ASSERT(_Buffer.buf!=nullptr, throw std::invalid_argument("_Buffer.buf is nullptr [1] (" __FUNCTION__ ")"));
...
template <class T>
T& ptr::_append_emplace(_CGD_BOUND_INFO_PARAM)
{
	// Check) 새로 추가되는 크기는 Upper Bound를 넘어서는 안됀다.
...
ptr::_append_array(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_reads_(_Count) const T* _Data, _In_ size_t _Count)
{
	...
	// 1) [데이터_갯수]를 얻어낸다.
	temp._append_general<COUNT_T>(_CGD_BOUND_INFO_PASS _CGD_COMMA static_cast<COUNT_T>(_Count));
...
inline void* ptr::_append_bytes(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ size_t _Size, _In_reads_bytes_(_Size) const void* _Buffer)
{
...
void* ptr::_append_string_pointer(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_z_ const T* _String)
{
	...
	// 3) [문자열_길이]를 저장한다.
	*reinterpret_cast<COUNT_T*>(m_ptr)	 = static_cast<COUNT_T>(lengthString);
...
template <class T, class... TREST>
void* ptr::_append_string_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest)
{
	// 1) [문자열]을 복사할 위치를 구한다.
	char*	strDest	 = m_ptr;

	// 2) [문자열]을 복사한다.
	const size_t	lengthString	 = _Xappend_string_format(_CGD_BOUND_INFO_PASS _CGD_COMMA(T*)(m_ptr + sizeof(COUNT_T)), _Format, std::forward<TREST>(_Rest)...);
	...
	*reinterpret_cast<COUNT_T*>(m_ptr)	 = static_cast<COUNT_T>(lengthString);
	...
	// Return) 
	return	reinterpret_cast<T*>(strDest);
}
...
template <class T, class... TREST>
size_t ptr::_append_text_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest)
{
	// Check) 새로 추가되는 크기는 Upper Bound를 넘어서는 안됀다.
	_CGD_BUFFER_BOUND_CHECK((m_ptr)>=_bound.bound_lower);
...
inline void* ptr::_append_buffer(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const buffer& _Buffer)
{
	...
	// Check) _Buffer.buf이 nullptr이면 안된다.
	...
	// Check) 새로 추가되는 크기는 Upper Bound를 넘어서는 안됀다.
	_CGD_BUFFER_BOUND_CHECK((m_ptr) >= _bound.bound_lower && (m_ptr + sizeof(uint32_t) + _Buffer.len) <= _bound.bound_upper);
...
COUNT_T ptr::_extract_string_copy(_CGD_BOUND_INFO_PARAM _CGD_COMMA __out_ecount(_LengthInWords) T* _String, _In_ size_t _LengthInWords)
{
	...
	COUNT_T	lengthString	 = *reinterpret_cast<COUNT_T*>(m_ptr);
	...
	if(*(T*)(m_ptr+sizeString+sizeof(COUNT_T)-sizeof(T)) != T(NULL)) throw std::length_error("it's invalid string!! [6] (" __FUNCTION__ ")");
...
T* ptr::_extract_string_pointer(_CGD_BOUND_INFO_PARAM)
{
	...
	// 1) [문자열_길이]를 얻어낸다.(제일 처음 4Byte가 문자열의 길이다.)
	size_t	sizeString	 = static_cast<size_t>(*reinterpret_cast<COUNT_T*>(m_ptr));
	...
	T*	strString	 = reinterpret_cast<T*>(m_ptr+sizeof(COUNT_T));
...
std::basic_string<T> ptr::_extract_text(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ T _CharTerminal)
{
	...
	// Declare)
	T*			chNow	 = m_ptr;
	const T*	pEnd	 = reinterpret_cast<const T*>(UINTPTR_MAX);
...
inline char* ptr::_extract_web_modify(_CGD_BOUND_INFO_PARAM)
{
	...
	// Declare)
	char*		chNow	 = m_ptr;
	const char*	pEnd	 = reinterpret_cast<const char*>(UINTPTR_MAX);
	...
	for(; chNow<pEnd; ++chNow)
	{
		if(*(uint16_t*)chNow==wCompare)
		{
			...
			*(p_pstrBuffer+ilength)	 = char(NULL);
			...
			// - m_ptr를 chNow+2로 설정한다. (/r/n부분을 빼기위해+2를 함.)
			m_ptr	 = (chNow+2);
			//len		 = static_cast<COUNT_T>(pEnd-chNow-1);
...
inline void * ptr::_extract_bytes(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ size_t _Size, _In_ void* _Buffer)
{
	// Check) _Buffer이 nullptr이면 안된다.
...
T& ptr::_front_general(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const
{
	...
	T* p	 = reinterpret_cast<T*>(m_ptr+_Offset);
...
T* ptr::_front_string_pointer(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const 
{
	...
	// 1) [문자열_길이]를 얻어낸다.(제일 처음 4Byte가 문자열의 길이다.)
	size_t	lengthString	 = static_cast<size_t>(*reinterpret_cast<COUNT_T*>(m_ptr+_Offset));
	...
	// Check) 문자열의 제일 끝이 NULL인지 확인한다.
	if(*(T*)(m_ptr+posTail-sizeof(T))!=T(NULL)) throw std::out_of_range("Invalid string! [2] (" __FUNCTION__ ")");
	
	// 3) Offset을 더한다.
	_Offset		 = static_cast<int>(posTail);

	// Return) String의 위치를 return한다.
	return	reinterpret_cast<T*>(m_ptr+posStart);
}
...
std::basic_string<T>  ptr::_front_string(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const
{
	...
	// Check) 문자열의 제일 끝이 NULL인지 확인한다.
	if(*(T*)(m_ptr+posTail-sizeof(T))!=T(NULL)) throw std::out_of_range("string is too short!! [3] (" __FUNCTION__ ")");

	// 4) 최종적으로 계산된 Offset값을 업데이트한다.
	_Offset		 = static_cast<int>(posTail);

	// Return) [문자열]을 리턴한다. (RVO)
	return	std::basic_string<T>(reinterpret_cast<T*>(m_ptr+posStart));
}
...
inline char* ptr::_front_web(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const
{
	...
	const char*	pEnd	 = m_ptr+_Offset+(size_t(0)-1);
	...
	for(; chNow<pEnd; ++chNow)
	{
		if(*(uint16_t*)chNow==wCompare)
		{
			...
			// 5) Offset을 써넣는다.
			_Offset	 = static_cast<int>(chNow+2-m_ptr);
