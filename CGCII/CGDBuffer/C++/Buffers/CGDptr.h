...
public:
	...
	// 5) extract/subtract
			...
			template <class T>
			size_t				extract(__out_ecount(_Length_in_words) T* _String, _In_ size_t _Length_in_words=(size_t(0)-1)) {	return _extract_string_copy(_CGD_BOUND_INFO_GET _CGD_COMMA _String, _Length_in_words);}
			template<class... T>
			size_t				extract(const std::tuple<T...>& _Tuple)	{	auto temp=m_ptr; _extract_tuple(_CGD_BOUND_INFO_GET _CGD_COMMA _Tuple); return m_ptr-temp;}
	...
	// 8) Operator Overloading
			...
			// [operator] +=/-=
			ptr&				operator+=(const buffer& _Rhs)			{	_append_bytes(_CGD_BOUND_INFO_GET _CGD_COMMA _Rhs.len, _Rhs.buf); return *this;}
...
public:
	// Push Front) 
			...
			void*				_prepend_string_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest);
			template <class T>
			...
			template <class T, class... TREST>
			size_t				_prepend_text_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest);
	...
	// Push Back) 
			...
			template <class T, class... TREST>
			void*				_append_string_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest);
			template <class T>
			...
			template <class T, class... TREST>
			size_t				_append_text_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest);
