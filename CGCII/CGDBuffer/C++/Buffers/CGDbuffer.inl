...
inline void* buffer::_prepend_skip(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ size_t _Size)
{
	...
	len	 += static_cast<uint32_t>(_Size);
...
T& buffer::_prepend_general(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T& _Data)
{
	...
	*reinterpret_cast<T*>(buf-sizeof(T))	 = _Data;
...
buffer::_prepend_array(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_reads_(_Count) const T* _Data, _In_ size_t _Count)
{
	...
	_prepend_bytes(_CGD_BOUND_INFO_PASS _CGD_COMMA sizeof(T)*_Count, _Data);

	// 2) [데이터_갯수]를 써넣는다.
	_prepend_general<COUNT_T>(_CGD_BOUND_INFO_PASS _CGD_COMMA static_cast<COUNT_T>(_Count));
...
inline void* buffer::_prepend_bytes(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ size_t _Size, _In_reads_bytes_(_Size) const void* _Buffer)
{
	// Check) _Size가 0이하면 안된다.
	CGD_ASSERT(_Size>=0, throw std::invalid_argument("_Size is invalid [0] (" __FUNCTION__ ")"));
...
void* buffer::_prepend_string_pointer(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_reads_z_(_Buffer_Size) const T* _String, _In_ size_t _Buffer_Size)
{
	...
	// 2) [문자열]을 복사한다.
	_prepend_bytes(_CGD_BOUND_INFO_PASS _CGD_COMMA sizeString, _String);

	// 3) Size를 넣는다.
	_prepend_general<COUNT_T>(_CGD_BOUND_INFO_PASS _CGD_COMMA static_cast<COUNT_T>(lengthString));
...
{
	// 1) String길이를 구한다.(NULL길이까지 포함하므로 +1한다.)
	size_t	lengthString = static_cast<size_t>(_String.length())+1;
	size_t	sizeString	 = lengthString*sizeof(T);
	...
	_prepend_bytes(_CGD_BOUND_INFO_PASS _CGD_COMMA sizeString, _String.c_str());
...
void* buffer::_prepend_const_string(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const CGD::const_string<T,N>& _constString)
{
	...
	// 3) [문자열_길이]를 저장한다.
	*reinterpret_cast<COUNT_T*>(p) = static_cast<COUNT_T>(lengthString);
	...
	buf -= static_cast<uint32_t>(sizeString);
	len += static_cast<uint32_t>(sizeString);
...
buffer::_prepend_string_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest)
{
#ifdef _WIN32
	...
	// 1) [임시_버퍼]를 할당한다
#ifdef _CGMEM_POOL_ALLOC
	CGPTR<ICGBuffer>	pbufferTemp	 = MEM_POOL_ALLOC(TEMP_STRING_BUFFER_SIZE);
	T*	pStringStart = reinterpret_cast<T*>(pbufferTemp->buf);
#else 
	T*	pStringStart = reinterpret_cast<T*>(CGMALLOC(TEMP_STRING_BUFFER_SIZE));
#endif
	// 3) [원본_버퍼_시작_크기]에 [형식_문자열]를 써넣는다.
	size_t	lengthString = _Xsprintf(pStringStart, _Format, std::forward<TREST>(_Rest)...)+1;
	size_t	sizeString	 = lengthString*sizeof(T);
	...
	*reinterpret_cast<COUNT_T*>(p)	 = static_cast<COUNT_T>(lengthString);
	...
	buf	 = p;
	len	+= static_cast<uint32_t>(sizeString+sizeof(COUNT_T));
...
buffer::_prepend_text_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest)
{
#ifdef _WIN32
	// Check) _String이 nullptr이면 안된다.
	CGD_ASSERT(_Format!=nullptr, throw std::invalid_argument("_Format is nullptr! [0] (" __FUNCTION__ ")"));
	...
	size_t	lengthString	 = _Xsprintf(pStringStart, std::forward<TREST>(_Rest)...);
	size_t	sizeString		 = lengthString*sizeof(T);
...
{
	// Check) _Buffer.len가 0이하면 안된다.
	CGD_ASSERT(_Buffer.len>=0, throw std::invalid_argument("_Buffer.len is invalid [0] (" __FUNCTION__ ")"));

	// 1) [데이터_갯수]만큼 데이터들을 통채로 복사한다.
	_prepend_bytes(_CGD_BOUND_INFO_PASS _CGD_COMMA _Buffer.len, _Buffer.buf);

	// 2) [데이터_갯수]를 써넣는다.
	_prepend_general<uint32_t>(_CGD_BOUND_INFO_PASS _CGD_COMMA _Buffer.len);
...
inline void* buffer::_append_skip(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ size_t _Length)
{
	...
	len		+= static_cast<uint32_t>(_Length);
...
#pragma pop_macro("new")
	...
	len += static_cast<uint32_t>(sizeof(T));
...
buffer::_append_array(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_reads_(_Count) const T* _Data, _In_ size_t _Count)
{
	...
	CGD_ASSERT(_Data!=nullptr || (_Data==nullptr && _Count==0), throw std::invalid_argument("_Data is nullptr [1] (" __FUNCTION__ ")"));
	...
	// 1) [데이터_갯수]를 써넣는다.
	_append_general<COUNT_T>(_CGD_BOUND_INFO_PASS _CGD_COMMA static_cast<COUNT_T>(_Count));

	// 2) [데이터]들을 통채로 복사한다.
	_append_bytes(_CGD_BOUND_INFO_PASS _CGD_COMMA _Count*sizeof(T), _Data);
...
inline void* buffer::_append_bytes(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ size_t _Size, _In_reads_bytes_(_Size) const void* _Buffer)
{
	// Check) _Size가 0이하면 안된다.
	CGD_ASSERT(_Size>=0, throw std::invalid_argument("_Size is invalid [0] (" __FUNCTION__ ")"));

	// Check) _Buffer이 nullptr이면 안된다.
	CGD_ASSERT(_Buffer!=nullptr || (_Buffer==nullptr && _Size==0), throw std::invalid_argument("_Buffer is nullptr [1] (" __FUNCTION__ ")"));
...
void* buffer::_append_string_pointer(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_z_ const T* _String)
{
	...
	// 2) [문자열]을 복사한다.
	size_t	lengthString	 = _Xappend_string<T>(_CGD_BOUND_INFO_PASS _CGD_COMMA reinterpret_cast<T*>(strDest+sizeof(COUNT_T)), _String);

	// 3) [문자열_길이]를 저장한다.
	*reinterpret_cast<COUNT_T*>(buf+len)	 = static_cast<COUNT_T>(lengthString);

	// 4) [버퍼_길이]를 업데이트한다.
	len		+= static_cast<uint32_t>(lengthString*sizeof(T)+sizeof(COUNT_T));
...
void* buffer::_append_string(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const std::basic_string<T>& _String)
{
	...
	size_t	lengthString	 = _Xappend_string(_CGD_BOUND_INFO_PASS _CGD_COMMA reinterpret_cast<T*>(strDest+sizeof(COUNT_T)), _String.c_str(), _String.length());
...
template <class T, class... TREST>
typename std::enable_if<is_string_type<T>::value, void*>::type
buffer::_append_string_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest)
{
	// 1) [문자열]을 복사할 위치를 구한다.
	char*	strDest	 = buf+len;

	// 2) [문자열]을 복사한다.
	const size_t	lengthString	 = _Xappend_string_format(_CGD_BOUND_INFO_PASS _CGD_COMMA reinterpret_cast<T*>(strDest+sizeof(COUNT_T)), _Format, std::forward<TREST>(_Rest)...);
...
template <class T, class... TREST>
typename std::enable_if<is_string_type<T>::value, size_t>::type
buffer::_append_text_format(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ const T* _Format, TREST&&... _Rest)
{
	// Check) 새로 추가되는 크기는 Upper Bound를 넘어서는 안됀다.
	_CGD_BUFFER_BOUND_CHECK((buf+len)>=_bound.bound_lower);

	// 1) [문자열]을 복사할 위치를 저장한다.
	T*	strDest = reinterpret_cast<T*>(buf + len);
	...
	const size_t	lengthString = _Xsprintf(strDest, _Format, std::forward<TREST>(_Rest)...);
...
{
	// Check) 새로 추가되는 크기는 Upper Bound를 넘어서는 안됀다.
	_CGD_BUFFER_BOUND_CHECK((buf+len)>=_bound.bound_lower && (buf+len+sizeof(uint32_t)+_Buffer.len)<=_bound.bound_upper);

	// Check) _Count가 0이하면 안된다.
	CGD_ASSERT(_Buffer.len>=0, throw std::invalid_argument("_Buffer.len is invalid [0] (" __FUNCTION__ ")"));

	// Check) _Data이 nullptr이면 안된다.
	CGD_ASSERT(_Buffer.buf!=nullptr || (_Buffer.buf==nullptr && _Buffer.len==0), throw std::invalid_argument("_Data is nullptr [1] (" __FUNCTION__ ")"));

	// 1) [원본_버퍼_포인터]를 저장해 놓는다.
	char*	p	 = buf+len;

	// 2) 데이터를 써넣는다.
	if(_Buffer.buf != nullptr && _Buffer.len != 0)
	{
		// - [데이터_갯수]를 써넣는다.
		_append_general<uint32_t>(_CGD_BOUND_INFO_PASS _CGD_COMMA _Buffer.len);

		// - [데이터]들을 통채로 복사한다.
		_append_bytes(_CGD_BOUND_INFO_PASS _CGD_COMMA _Buffer.len, _Buffer.buf);
	}
	else
	{
		// 2) [데이터_갯수]를 0으로 써넣는다.
		_append_general<uint32_t>(_CGD_BOUND_INFO_PASS _CGD_COMMA 0);
	}

	// Return) 
	return	reinterpret_cast<void*>(p+sizeof(uint32_t));
}
...
template <class T>
COUNT_T buffer::_extract_string_copy(_CGD_BOUND_INFO_PARAM _CGD_COMMA __out_ecount(_LengthInWords) T* _String, _In_ size_t _LengthInWords)
{
	// Check) _String이 nullptr이면 안된다.
	CGD_ASSERT(_String!=nullptr, throw std::invalid_argument("_String is nullptr [1] (" __FUNCTION__ ")"));
	...
	if(sizeString>= _LengthInWords) throw std::length_error("buffer size is too small!! [5] (" __FUNCTION__ ")");
...
T* buffer::_extract_string_pointer(_CGD_BOUND_INFO_PARAM)
{
	...
	{
		buf		+= sizeof(COUNT_T);
		len		-= static_cast<uint32_t>(sizeof(COUNT_T));
		return	nullptr;
	}

	// 2) 문자열의 제일 앞 포인터를 얻어둔다.
	T*	strString	 = reinterpret_cast<T*>(buf+sizeof(COUNT_T));

	// 2) sizeOfData 구하기
	sizeData	 = static_cast<COUNT_T>(sizeData*sizeof(T)+sizeof(COUNT_T));

	// 3) [원본_버퍼_포인터]를 [데이터_크기]만큼 더한다.
	buf	+= sizeData;

	// 4) [원본_버퍼_길이]를 [데이터_크기]만큼 줄인다.
	len	-= static_cast<uint32_t>(sizeData);
...
inline void* buffer::_extract_bytes(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ size_t _Size, _In_ void* _Buffer)
{
	// Check) _Buffer이 nullptr이면 안된다.
...
template <class T>
std::basic_string<T> buffer::_extract_text(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ T _CharTerminal)
{
	// Check) Lower Bound 확인
	_CGD_BUFFER_BOUND_CHECK((buf)>=_bound.bound_lower);

	// Declare)
	T*			chNow	 = reinterpret_cast<T*>(buf);
	const T*	pEnd	 = reinterpret_cast<T*>(buf+len);
	...
	for(; chNow<pEnd; ++chNow)
	{
		if(*chNow==_CharTerminal)
		{
			...
			// - buf를 chNow+1로 설정한다. (Terminal 문제 부분을 빼기위해+2를 함.)
			buf		 = (chNow+1);
			len		 = static_cast<uint32_t>((uintptr_t)pEnd-(uintptr_t)chNow);

			// Return) 
			return	std::basic_string<T>(pHead, pEnd-chNow);
		}
	}
...
template <class T>
T* buffer::_extract_text(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ T _CharTerminal,  _In_ T _CharChange)
{
	// Check) Lower Bound 확인
	_CGD_BUFFER_BOUND_CHECK((buf)>=_bound.bound_lower);
	...
	for(; chNow<pEnd; ++chNow)
	{
		if(*chNow==_CharTerminal)
		{
			...
			// - buf를 chNow+2로 설정한다. (Terminal 문제부분을 빼기위해+2를 함.)
			buf		 = (chNow+1);
			len		 = static_cast<uint32_t>((uintptr_t)pEnd-(uintptr_t)chNow);
...
inline char* buffer::_extract_web_modify(_CGD_BOUND_INFO_PARAM)
{
	// Check) Lower Bound 확인
	_CGD_BUFFER_BOUND_CHECK((buf)>=_bound.bound_lower);
	...
	for(; chNow<pEnd; ++chNow)
	{
		if(*(uint16_t*)chNow==wCompare)
		{
			...
			buf		 = (chNow+2);
			len		 = static_cast<uint32_t>(pEnd-chNow-1);
...
inline std::string buffer::_extract_web(_CGD_BOUND_INFO_PARAM)
{
	// Check) Lower Bound 확인
	_CGD_BUFFER_BOUND_CHECK((buf)>=_bound.bound_lower);
...
COUNT_T buffer::_get_front_string_length(_CGD_BOUND_INFO_PARAM )
{
	...
	if(sizeof(COUNT_T)>static_cast<size_t>(len)) throw std::length_error("buffer is too short [0] (" __FUNCTION__ ")");

	// 1) [문자열_길이]를 얻어낸다.(제일 처음 4Byte가 문자열의 길이다.)
	const COUNT_T	lengthString	 = *reinterpret_cast<COUNT_T*>(buf);
	...
	if(sizeData>static_cast<size_t>(len)) throw std::length_error("string is too short!! [2] (" __FUNCTION__ ")");

	// Check) 문자열의 제일 끝이 NULL인지 확인한다.
	if(*reinterpret_cast<T*>(buf+sizeData-sizeof(T)) != T(NULL)) throw std::length_error("it's invalid string! [3] (" __FUNCTION__ ")");
...
inline buffer buffer::_begin(_CGD_BOUND_INFO_PARAM _CGD_COMMA _In_ int _Offset) const	
{
	...
	return buffer(buf+_Offset, (len>static_cast<uint32_t>(_Offset)) ? len-static_cast<uint32_t>(_Offset) : 0);
}
...
T& buffer::_front_general(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const
{
	...
	T* p	 = reinterpret_cast<T*>(buf+_Offset);
	
	// 2) Offset을 Sizeof(T)증가시킨다.
	_Offset	+= static_cast<int>(sizeof(T));
...
T* buffer::_front_string_pointer(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const 
{
	...
	if((_Offset+sizeof(COUNT_T))>static_cast<uint32_t>(len)) throw std::out_of_range("string is too short!! [0] (" __FUNCTION__ ")");

	// 1) [문자열_길이]를 얻어낸다.(제일 처음 4Byte가 문자열의 길이다.)
	size_t	lengthString	 = static_cast<size_t>(*reinterpret_cast<COUNT_T*>(buf+_Offset));
	...
	if(posTail>static_cast<size_t>(len)) throw std::out_of_range("string is to long!! [2] (" __FUNCTION__ ")");

	// Check) 문자열의 제일 끝이 NULL인지 확인한다.
	if(*reinterpret_cast<T*>(buf+posTail-sizeof(T))!=T(NULL)) throw std::out_of_range("Invalid string! [3] (" __FUNCTION__ ")");
	
	// 4) 최종적으로 계산된 Offset값을 업데이트한다.
	_Offset		 = static_cast<int>(posTail);

	// Return) [문자열_시작_포인터]을 리턴한다.
	return	reinterpret_cast<T*>(buf+posStart);
}
...
std::basic_string<T> buffer::_front_string(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const
{
	...
	// Check) [원본_버퍼_길이]가 최소한 sizeof(unit32_t)보다는 길어야 한다.
	if((_Offset+sizeof(COUNT_T)+sizeof(T))>static_cast<int>(len)) throw std::length_error("string is too short!! [0] (" __FUNCTION__ ")");
	...
	// Check) 문자열의 제일 끝이 NULL인지 확인한다.
	if(*reinterpret_cast<T*>(buf+posTail-sizeof(T))!=T(NULL)) throw std::out_of_range("string is too short!! [3] (" __FUNCTION__ ")");
	
	// 4) 최종적으로 계산된 Offset값을 업데이트한다.
	_Offset		 = static_cast<int>(posTail);

	// Return) [문자열]을 리턴한다. (RVO)
	return	std::basic_string<T>(reinterpret_cast<T*>(buf+posStart));
}
...
inline char* buffer::_front_web(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const
{
	...
	// Check) [문자열_길이]가 최소 문자열의 길이(NULL문자)의 크기 작으면 오류다!
	if(static_cast<uint32_t>(_Offset)>(len-1)) throw std::out_of_range("_Offset is Out of Range!! [0] (" __FUNCTION__ ")");
	...
	for(; chNow<pEnd; ++chNow)
	{
		if(*(uint16_t*)chNow==wCompare)
		{
			...
			// 5) Offset을 써넣는다.
			_Offset	 = static_cast<int>(chNow+2-buf);
...
T& buffer::_back_general(_CGD_BOUND_INFO_PARAM _CGD_COMMA _Inout_ int& _Offset) const
{
	...
	T* p	 = reinterpret_cast<T*>(buf+len+_Offset);
...
{
	// Declare) 
	CGD::buffer	bufferTemp;

	// 1) [데이터_갯수를 먼저 읽어들인다.]
	bufferTemp.len = _Source->TEMPLATE _extract<uint32_t>(_CGD_BOUND_INFO_PASS);

	// 2) 위치를 넣는다.
	bufferTemp.buf = _Source->get_front_ptr();

	// 3) [데이터_바이트수]만큼 
	_Source->TEMPLATE _extract_skip(_CGD_BOUND_INFO_PASS _CGD_COMMA bufferTemp.len);
