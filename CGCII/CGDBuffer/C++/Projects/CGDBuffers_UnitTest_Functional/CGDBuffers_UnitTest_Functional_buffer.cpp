...
{		
	...
	public:
		TEST_METHOD(Test_buffer_DateTime)
		{
			// X) Memory Alloc
			...
			dtA.wMinute		 = 10;
			dtA.wSecond		 = 20;
			dtA.dwMillisecond = 121;

			DATETIME	dtB;

			// - 값 써넣기
			bufferTemp.append<DATETIME>(dtA);
			Assert::IsTrue(bufferTemp.len == 16);
			Assert::IsTrue(bufferTemp.len == SizeOf(dtA));
			dtB = bufferTemp.extract<DATETIME>();
			Assert::IsTrue(bufferTemp.len == 0);

			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(dtB.wYear == dtA.wYear);
			Assert::IsTrue(dtB.wMonth == dtA.wMonth);
			Assert::IsTrue(dtB.wDay == dtA.wDay);
			Assert::IsTrue(dtB.wHour == dtA.wHour);
			Assert::IsTrue(dtB.wMinute == dtA.wMinute);
			Assert::IsTrue(dtB.wSecond == dtA.wSecond);
			Assert::IsTrue(dtB.dwMillisecond == dtA.dwMillisecond);
		#endif
			...
			Assert::IsTrue(bufferTemp.len == 16);
			Assert::IsTrue(bufferTemp.len == SizeOf(dtA));
			stB	 = bufferTemp.extract<SYSTEMTIME>();
			Assert::IsTrue(bufferTemp.len == 0);

			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(stB.wYear == dtA.wYear);
			Assert::IsTrue(stB.wMonth == dtA.wMonth);
			Assert::IsTrue(stB.wDay == dtA.wDay);
			Assert::IsTrue(stB.wDayOfWeek == GetDayOfWeek(dtA.wYear, dtA.wMonth, dtA.wDay));
			Assert::IsTrue(stB.wHour == dtA.wHour);
			Assert::IsTrue(stB.wMinute == dtA.wMinute);
			Assert::IsTrue(stB.wSecond == dtA.wSecond);
			Assert::IsTrue(stB.wMilliseconds == dtA.dwMillisecond);
		#endif
			...
			Assert::IsTrue(bufferTemp.len == 16);
			Assert::IsTrue(bufferTemp.len == SizeOf(dtA));
			tmC = bufferTemp.extract<tm>();
			Assert::IsTrue(bufferTemp.len == 0);

			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(tmC.tm_year + 1900 == dtA.wYear);
			Assert::IsTrue(tmC.tm_mon == dtA.wMonth);
			Assert::IsTrue(tmC.tm_mday == dtA.wDay);
			Assert::IsTrue(tmC.tm_hour == dtA.wHour);
			Assert::IsTrue(tmC.tm_min == dtA.wMinute);
			Assert::IsTrue(tmC.tm_sec == dtA.wSecond);
		#endif


			TIME::LOCAL::time_point	tpE;

			// - 값 써넣기
			bufferTemp.append<DATETIME>(dtA);
			Assert::IsTrue(bufferTemp.len == 16);
			Assert::IsTrue(bufferTemp.len == SizeOf(dtA));
			tpE = bufferTemp.extract<TIME::LOCAL::time_point>();
			Assert::IsTrue(bufferTemp.len == 0);

			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(tpE.get_year() == dtA.wYear);
			Assert::IsTrue(tpE.get_month() == dtA.wMonth);
			Assert::IsTrue(tpE.get_day() == dtA.wDay);
			Assert::IsTrue(tpE.get_hour() == dtA.wHour);
			Assert::IsTrue(tpE.get_minute() == dtA.wMinute);
			Assert::IsTrue(tpE.get_second() == dtA.wSecond);
			Assert::IsTrue(tpE.get_millisecond() == dtA.dwMillisecond);
		#endif

		}

		TEST_METHOD(Test_buffer_SYSTEMTIME)
		{
			// X) Memory Alloc
			char	tempBuffer[2048];
			...
			dtA.wMonth			 = 12;
			dtA.wDay			 = 11;
			dtA.wDayOfWeek		 = GetDayOfWeek(dtA.wYear, dtA.wMonth, dtA.wDay);
			dtA.wHour			 = 5;
			dtA.wMinute			 = 10;
			dtA.wSecond			 = 20;
			dtA.wMilliseconds	 = 121;

			DATETIME	dtB;

			// - 값 써넣기
			bufferTemp.append<SYSTEMTIME>(dtA);
			Assert::IsTrue(bufferTemp.len==16);
			Assert::IsTrue(bufferTemp.len == SizeOf(dtA));
			dtB = bufferTemp.extract<DATETIME>();
			Assert::IsTrue(bufferTemp.len == 0);
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(dtA.wYear== dtB.wYear);
			Assert::IsTrue(dtA.wMonth == dtB.wMonth);
			Assert::IsTrue(dtA.wDay == dtB.wDay);
			Assert::IsTrue(dtA.wHour == dtB.wHour);
			Assert::IsTrue(dtA.wMinute == dtB.wMinute);
			Assert::IsTrue(dtA.wSecond == dtB.wSecond);
			Assert::IsTrue(dtA.wMilliseconds == dtB.dwMillisecond);
		#endif
			...
			Assert::IsTrue(stB.wMinute == dtA.wMinute);
			Assert::IsTrue(stB.wSecond == dtA.wSecond);
			Assert::IsTrue(stB.wMilliseconds == dtA.wMilliseconds);
		#endif
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(dtA.wYear == tmC.tm_year+1900);
			Assert::IsTrue(dtA.wMonth == tmC.tm_mon);
			Assert::IsTrue(dtA.wDay == tmC.tm_mday);
			Assert::IsTrue(dtA.wHour == tmC.tm_hour);
			Assert::IsTrue(dtA.wMinute == tmC.tm_min);
			Assert::IsTrue(dtA.wSecond == tmC.tm_sec);
		#endif
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(dtA.wYear == tpE.get_year());
			Assert::IsTrue(dtA.wMonth == tpE.get_month());
			Assert::IsTrue(dtA.wDay == tpE.get_day());
			Assert::IsTrue(dtA.wHour == tpE.get_hour());
			Assert::IsTrue(dtA.wMinute == tpE.get_minute());
			Assert::IsTrue(dtA.wSecond == tpE.get_second());
			Assert::IsTrue(dtA.wMilliseconds == tpE.get_millisecond());
		#endif

		}

		TEST_METHOD(Test_buffer_tm)
		{
			// X) Memory Alloc
			char	tempBuffer[2048];
			...
			tm	dtA;
			dtA.tm_year			 = 2017-1900;
			dtA.tm_mon			 = 12;
			dtA.tm_mday			 = 11;
			dtA.tm_yday			 = GetDayOfYear(2017, dtA.tm_mon, dtA.tm_mday);
			dtA.tm_wday			 = GetDayOfWeek(2017, dtA.tm_mon, dtA.tm_mday);
			dtA.tm_hour			 = 5;
			dtA.tm_min			 = 10;
			dtA.tm_sec			 = 20;
			dtA.tm_isdst		 = 0;
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(dtB.wYear == dtA.tm_year + 1900);
			Assert::IsTrue(dtB.wMonth == dtA.tm_mon);
			Assert::IsTrue(dtB.wDay == dtA.tm_mday);
			Assert::IsTrue(dtB.wHour == dtA.tm_hour);
			Assert::IsTrue(dtB.wMinute == dtA.tm_min);
			Assert::IsTrue(dtB.wSecond == dtA.tm_sec);
			Assert::IsTrue(dtB.dwMillisecond == 0);
		#endif
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(stB.wYear == dtA.tm_year + 1900);
			Assert::IsTrue(stB.wMonth == dtA.tm_mon);
			Assert::IsTrue(stB.wDay == dtA.tm_mday);
			Assert::IsTrue(stB.wDayOfWeek == dtA.tm_wday);
			Assert::IsTrue(stB.wHour == dtA.tm_hour);
			Assert::IsTrue(stB.wMinute == dtA.tm_min);
			Assert::IsTrue(stB.wSecond == dtA.tm_sec);
			Assert::IsTrue(stB.wMilliseconds == 0);
		#endif
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(tmC.tm_year == dtA.tm_year);
			Assert::IsTrue(tmC.tm_mon == dtA.tm_mon);
			Assert::IsTrue(tmC.tm_mday == dtA.tm_mday);
			Assert::IsTrue(tmC.tm_yday == dtA.tm_yday);
			Assert::IsTrue(tmC.tm_wday == dtA.tm_wday);
			Assert::IsTrue(tmC.tm_hour == dtA.tm_hour);
			Assert::IsTrue(tmC.tm_min == dtA.tm_min);
			Assert::IsTrue(tmC.tm_sec == dtA.tm_sec);
			Assert::IsTrue(tmC.tm_isdst == dtA.tm_isdst);
		#endif
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(tpE.get_year() == dtA.tm_year + 1900);
			Assert::IsTrue(tpE.get_month() == dtA.tm_mon);
			Assert::IsTrue(tpE.get_day() == dtA.tm_mday);
			Assert::IsTrue(tpE.get_hour() == dtA.tm_hour);
			Assert::IsTrue(tpE.get_minute() == dtA.tm_min);
			Assert::IsTrue(tpE.get_second() == dtA.tm_sec);
			Assert::IsTrue(tpE.get_millisecond() == 0);
		#endif
		}

		TEST_METHOD(Test_buffer_timepoint)
		{
			// X) Memory Alloc
			char	tempBuffer[2048];
			...
			dtA.set_minute(10);
			dtA.set_second(20);
			dtA.set_millisecond(121);

			DATETIME	dtB;

			// - 값 써넣기
			bufferTemp.append<TIME::LOCAL::time_point>(dtA);
			Assert::IsTrue(bufferTemp.len == 16);
			Assert::IsTrue(bufferTemp.len == SizeOf(dtA));
			dtB = bufferTemp.extract<DATETIME>();
			Assert::IsTrue(bufferTemp.len == 0);
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(dtB.wYear == dtA.get_year());
			Assert::IsTrue(dtB.wMonth == dtA.get_month());
			Assert::IsTrue(dtB.wDay == dtA.get_day());
			Assert::IsTrue(dtB.wHour == dtA.get_hour());
			Assert::IsTrue(dtB.wMinute == dtA.get_minute());
			Assert::IsTrue(dtB.wSecond == dtA.get_second());
			Assert::IsTrue(dtB.dwMillisecond == dtA.get_millisecond());
		#endif
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(stB.wYear == dtA.get_year());
			Assert::IsTrue(stB.wMonth == dtA.get_month());
			Assert::IsTrue(stB.wDay == dtA.get_day());
			Assert::IsTrue(stB.wDayOfWeek == GetDayOfWeek(dtA.get_year(), dtA.get_month(), dtA.get_day()));
			Assert::IsTrue(stB.wHour == dtA.get_hour());
			Assert::IsTrue(stB.wMinute == dtA.get_minute());
			Assert::IsTrue(stB.wSecond == dtA.get_second());
			Assert::IsTrue(stB.wMilliseconds == dtA.get_millisecond());
		#endif
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(tmC.tm_year + 1900 == dtA.get_year());
			Assert::IsTrue(tmC.tm_mon == dtA.get_month());
			Assert::IsTrue(tmC.tm_mday == dtA.get_day());
			Assert::IsTrue(tmC.tm_yday == GetDayOfYear(dtA.get_year(), dtA.get_month(), dtA.get_day()));
			Assert::IsTrue(tmC.tm_wday == GetDayOfWeek(dtA.get_year(), dtA.get_month(), dtA.get_day()));
			Assert::IsTrue(tmC.tm_hour == dtA.get_hour());
			Assert::IsTrue(tmC.tm_min == dtA.get_minute());
			Assert::IsTrue(tmC.tm_sec == dtA.get_second());
			Assert::IsTrue(tmC.tm_isdst == 0);
		#endif
			...
			// Check) 
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(tpE.get_year() == dtA.get_year());
			Assert::IsTrue(tpE.get_month() == dtA.get_month());
			Assert::IsTrue(tpE.get_day() == dtA.get_day());
			Assert::IsTrue(tpE.get_hour() == dtA.get_hour());
			Assert::IsTrue(tpE.get_minute() == dtA.get_minute());
			Assert::IsTrue(tpE.get_second() == dtA.get_second());
			Assert::IsTrue(tpE.get_millisecond() == dtA.get_millisecond());
		#endif
		}
		...
		TEST_METHOD(Test_buffer_append_string)
		{
			...
			// Check)
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferTest.len == SizeOf(strTest));
		#endif
		...
		TEST_METHOD(Test_buffer_append_const_string)
		{
			...
			// Check)
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferTest.len == SizeOf("Test XX"));
		#endif
		...
		TEST_METHOD(Test_buffer_String2)
		{
			...
			//-----------------------------------------------------------------
			// X) Memory Alloc
			char*	tempBuffer1	 = new char[1024];
			char*	tempBuffer2	 = new char[1024];


			// Case A) 메모리 할당 (Allocating Memory)
			CGD::buffer	bufferWrite(tempBuffer1);
			...
			// Check)
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferWrite.len == SizeOf(listTest));
		#endif

			// Case B) 메모리 할당 (Allocation Memory)
			CGD::buffer	tempBuffer(tempBuffer2);
			...
			// Check)
		#ifdef _ENABLE_ASSERT
			//Assert::IsTrue(tempBuffer.len == SizeOf(listTest.begin(), listTest.end()));
		#endif
		...
		TEST_METHOD(Test_buffer_append_text)
		{
			...
			Assert::IsTrue(bufferTest.len == SizeOf(CGD::_text(strTest)));
		#endif
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferTemp.len == SizeOf(temp));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_array$wchar_t)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(array_string_wchar_t));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(array_string_wchar_t));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_vector$int)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(vector_int));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(vector_int));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_vector$string)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(vector_std_string));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(vector_std_string));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_list$int)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(list_int));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(list_int));
		#endif
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(list_std_string));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_list$vector$int)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(list_vector_int));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(list_vector_int));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_set$int)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(set_int));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(set_int));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_set$string)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(set_std_string));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(set_std_string));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_map$int$int)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(map_std_int_int));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(map_std_int_int));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_map$string$int)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(map_std_string_int));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(map_std_string_int));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_map$string$vector$int)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(map_std_string_vector_int));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(map_std_string_vector_int));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_map$string$vector$charP)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(map_std_vector_string));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(map_std_vector_string));
		#endif
		...
		TEST_METHOD(Test_buffer_append_CONTAINER_unoredered_map$string$int)
		{
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferWrite.len == SizeOf(unordered_map_std_string_int));
			#endif
			}
		}
		...
		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(unordered_map_std_string_int));
		#endif
		...
		// Test) int append / front 
		//-----------------------------------------------------------------
		TEST_METHOD(Test_buffer_buffer)
		{
			// X) Memory Alloc
			char	tempBuffer[2048];
			...
			bufferCreate.append(bufA);

		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == SizeOf(vector_int) + SizeOf(bufA));
		#endif

			// - extract
			auto	a	 = bufferCreate.extract<decltype(vector_int)>();
			auto	b	 = bufferCreate.extract<CGD::buffer>();

		#ifdef _ENABLE_ASSERT
			Assert::IsTrue(bufferCreate.len == 0);
		#endif
		}
		...
		TEST_METHOD(Test_buffer_map_tuple1)
		{
			...
			#ifdef _ENABLE_ASSERT
				//Assert::IsTrue(bufferTemp.len == SizeOf(std::tuple<int, const char*, float>(iValue, strValue, fValue)));
			#endif
			}
		...
		TEST_METHOD(Test_buffer_web)
		{
			...
			// 2) Append
			bufferCreate.append_text("Test");
			bufferCreate.append_text("Test %d %s %f\n", 100, "Test", 0.1f);
		}
		...
		TEST_METHOD(Test_buffer_append_extract_Serializable)
		{
			...
			public:
				...
				virtual	bool	ProcessSerializeIn(CGD::buffer& _Buffer) override
				{
				...
				virtual	bool	ProcessSerializeOut(CGD::buffer& _Buffer) override
				{
					_Buffer.append(m_ix);
					_Buffer.append(m_fy);
				...
				virtual size_t	ProcessSerializeSizeOf() const override
				{
					size_t	size	 = 0;
			...
			#ifdef _ENABLE_ASSERT
				Assert::IsTrue(bufferTemp.len == SizeOf(a));
			#endif
			}
		...
		//-----------------------------------------------------------------
		TEST_METHOD(Test_buffer_append_extract_Serializable2)
		{
			struct TEMP_OBJECT : public CGD::IBufferSerializable
			{
			public:
			...
			public:
				...
				DEFINE_CGD_BUFFER_SERIALIZE
				(
					// 1) 클래스의 이름.
					TEMP_OBJECT,

					// 2) Serialize할 멤버들의 List
					m_ix,
					m_fy,
					m_strZ
				)
