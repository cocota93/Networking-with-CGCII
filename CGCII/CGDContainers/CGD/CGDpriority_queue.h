...
//-----------------------------------------------------------------------------
//
// CGD::priority_queue<T>
//
// 1. CGD::priority_queue란!
//     CGD::array는 기본적인 형태는 모두 vector와 동일하지만 크기가 고정적이다
...
template <typename TDATA, typename TCONTAINER=vector<TDATA>, typename TPR=std::less<typename TCONTAINER::value_type>>
...
public:
	// 1) 생성자들 1
			priority_queue()									{}
	explicit priority_queue(const TCONTAINER& _Container) : m_container(_Container)	{}
			priority_queue(const TPR& _Pred, const TCONTAINER& _Cont) ;

	// 2) 생성자들 2 (template로 된거.. 부분 복사와 함께 새성~)
